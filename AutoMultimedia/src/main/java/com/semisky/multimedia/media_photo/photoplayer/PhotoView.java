package com.semisky.multimedia.media_photo.photoplayer;
...
public class PhotoView extends ImageView {
	...
	private void initBase() {
		...
		if (mFromInfo != null
				&& System.currentTimeMillis() - mInfoTime < MAX_ANIM_FROM_WAITE) {
			animaFrom(mFromInfo);
		}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (mAdjustViewBounds
				&& (float) drawableW / drawableH != (float) width / height) {

			float hScale = (float) height / drawableH;
			float wScale = (float) width / drawableW;

			float scale = hScale < wScale ? hScale : wScale;
			width = p.width == ViewGroup.LayoutParams.MATCH_PARENT ? width
					: (int) (drawableW * scale);
			height = p.height == ViewGroup.LayoutParams.MATCH_PARENT ? height
					: (int) (drawableH * scale);
		}
...
//				onUp();
	...
	private void onUp() {
		...
		mTmpMatrix.postTranslate(cx - mHalfBaseRectWidth, cy
				- mHalfBaseRectHeight);
...
//		mTmpMatrix.postRotate(mDegrees, cx, cy);
	...
	private boolean isImageCenterHeight(RectF rect) {
		return Math.abs(Math.round(rect.top)
				- (mWidgetRect.height() - rect.height()) / 2) < 1;
	}

	private boolean isImageCenterWidth(RectF rect) {
		return Math.abs(Math.round(rect.left)
				- (mWidgetRect.width() - rect.width()) / 2) < 1;
	}

	public interface OnPhotoViewRotateListener {
		void onRotate(float degrees, float focusX, float focusY);
	}

	private OnPhotoViewRotateListener onPhotoViewRotateListener;

	public void setOnPhotoViewRotateListener(
			OnPhotoViewRotateListener onPhotoViewRotateListener) {
		this.onPhotoViewRotateListener = onPhotoViewRotateListener;
	}
	...
	private RotateGestureDetector.OnRotateListener mRotateListener = new RotateGestureDetector.OnRotateListener() {
		...
		public void onRotate(float degrees, float focusX, float focusY) {
			...
			if (canRotate) {
				...
				if (onPhotoViewRotateListener != null) {
					onPhotoViewRotateListener.onRotate(degrees, focusX, focusY);
				}
			} else {
	...
	private GestureDetector.OnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
		...
		public boolean onScroll(MotionEvent e1, MotionEvent e2,
			...
			if (canScrollHorizontallySelf(distanceX)) {
				if (distanceX < 0
						&& mImgRect.left - distanceX > mWidgetRect.left)
					distanceX = mImgRect.left;
				if (distanceX > 0
						&& mImgRect.right - distanceX < mWidgetRect.right)
				...
				if (!hasMultiTouch) {
					if (distanceX < 0
							&& mImgRect.left - distanceX > mCommonRect.left)
						distanceX = resistanceScrollByX(mImgRect.left
								- mCommonRect.left, distanceX);
					if (distanceX > 0
							&& mImgRect.right - distanceX < mCommonRect.right)
						distanceX = resistanceScrollByX(mImgRect.right
								- mCommonRect.right, distanceX);
				}
			...
			if (canScrollVerticallySelf(distanceY)) {
				...
				if (distanceY > 0
						&& mImgRect.bottom - distanceY < mWidgetRect.bottom)
				...
				if (!hasMultiTouch) {
					if (distanceY < 0
							&& mImgRect.top - distanceY > mCommonRect.top)
						distanceY = resistanceScrollByY(mImgRect.top
								- mCommonRect.top, distanceY);
					if (distanceY > 0
							&& mImgRect.bottom - distanceY < mCommonRect.bottom)
						distanceY = resistanceScrollByY(mImgRect.bottom
								- mCommonRect.bottom, distanceY);
				}
...
//            isZoonUp = !isZoonUp;
	...
	public boolean canScrollHorizontallySelf(float direction) {
		if (mImgRect.width() <= mWidgetRect.width())
			return false;
		if (direction < 0
				&& Math.round(mImgRect.left) - direction >= mWidgetRect.left)
			return false;
		if (direction > 0
				&& Math.round(mImgRect.right) - direction <= mWidgetRect.right)
			return false;
		return true;
	}
	...
	public boolean canScrollVerticallySelf(float direction) {
		if (mImgRect.height() <= mWidgetRect.height())
			return false;
		if (direction < 0
				&& Math.round(mImgRect.top) - direction >= mWidgetRect.top)
			return false;
		if (direction > 0
				&& Math.round(mImgRect.bottom) - direction <= mWidgetRect.bottom)
			return false;
		return true;
	}
	...
	private class Transform implements Runnable {
		...
		void withScale(float form, float to) {
			mScaleScroller.startScroll((int) (form * 10000), 0,
					(int) ((to - form) * 10000), 0, mAnimaDuring);
		}

		void withClip(float fromX, float fromY, float deltaX, float deltaY,
				int d, ClipCalculate c) {
			mClipScroller.startScroll((int) (fromX * 10000),
					(int) (fromY * 10000), (int) (deltaX * 10000),
					(int) (deltaY * 10000), d);
			C = c;
		}
		...
		void withRotate(int fromDegrees, int toDegrees) {
			mRotateScroller.startScroll(fromDegrees, 0,
					toDegrees - fromDegrees, 0, mAnimaDuring);
		}
		...
		void withFling(float velocityX, float velocityY) {
			...
			distanceX = velocityX < 0 ? Integer.MAX_VALUE - distanceX
					: distanceX;
			...
			distanceY = velocityY < 0 ? Integer.MAX_VALUE - distanceY
					: distanceY;
			...
			mFlingScroller.fling(mLastFlingX, mLastFlingY, (int) velocityX,
					(int) velocityY, minX, maxX, minY, maxY,
					Math.abs(overX) < MAX_FLING_OVER_SCROLL * 2 ? 0
...
//			mAnimaMatrix.postRotate(mDegrees, mRotateCenter.x, mRotateCenter.y);
	...
	public Info getInfo() {
		...
		int[] p = new int[2];
		getLocation(this, p);
		rect.set(p[0] + mImgRect.left, p[1] + mImgRect.top, p[0]
				+ mImgRect.right, p[1] + mImgRect.bottom);
		return new Info(rect, mImgRect, mWidgetRect, mBaseRect, mScreenCenter,
				mScale, mDegrees, mScaleType);
	}
	...
	public static Info getImageViewInfo(ImageView imgView) {
		...
		RectF rect = new RectF(p[0] + imgRect.left, p[1] + imgRect.top, p[0]
				+ imgRect.right, p[1] + imgRect.bottom);
		RectF widgetRect = new RectF(0, 0, imgView.getWidth(),
				imgView.getHeight());
		RectF baseRect = new RectF(widgetRect);
		PointF screenCenter = new PointF(widgetRect.width() / 2,
				widgetRect.height() / 2);

		return new Info(rect, imgRect, widgetRect, baseRect, screenCenter, 1,
				0, imgView.getScaleType());
	}
	...
	public void animaFrom(Info info) {
		if (isInit) {
			...
			mTranslate.withTranslate(0, 0, (int) -(ocx - mcx),
					(int) -(ocy - mcy));
			mTranslate.withScale(scale, 1);
			mTranslate.withRotate((int) info.mDegrees, 0);

			if (info.mWidgetRect.width() < info.mImgRect.width()
					|| info.mWidgetRect.height() < info.mImgRect.height()) {
				float clipX = info.mWidgetRect.width() / info.mImgRect.width();
				float clipY = info.mWidgetRect.height()
						/ info.mImgRect.height();
				clipX = clipX > 1 ? 1 : clipX;
				clipY = clipY > 1 ? 1 : clipY;

				ClipCalculate c = info.mScaleType == ScaleType.FIT_START ? new START()
						: info.mScaleType == ScaleType.FIT_END ? new END()
								: new OTHER();
	...
	public void animaTo(Info info, Runnable completeCallBack) {
		if (isInit) {
			...
			mScaleCenter.set(mImgRect.left + mImgRect.width() / 2, mImgRect.top
					+ mImgRect.height() / 2);
			mRotateCenter.set(mScaleCenter);
...
//			mAnimaMatrix.postRotate(mDegrees, mScaleCenter.x, mScaleCenter.y);
			...
			mTranslate.withRotate((int) mDegrees, (int) info.mDegrees,
					mAnimaDuring * 2 / 3);

			if (info.mWidgetRect.width() < info.mRect.width()
					|| info.mWidgetRect.height() < info.mRect.height()) {
				...
				final ClipCalculate c = info.mScaleType == ScaleType.FIT_START ? new START()
						: info.mScaleType == ScaleType.FIT_END ? new END()
								: new OTHER();
				...
				postDelayed(new Runnable() {
					@Override
					public void run() {
						mTranslate.withClip(1, 1, -1 + cx, -1 + cy,
								mAnimaDuring / 2, c);
					}
				}, mAnimaDuring / 2);
			}
    ...
    public void handScale(float scaleFactor) {
        //16 x 为最大倍数，当放大到16x直接重置 todo 2018-4-9
        if (mScale>=16){
            reset();
            return;
        }
        ...
        mScale *= scaleFactor;
        // mScaleCenter.set(detector.getFocusX(), detector.getFocusY());
        mAnimaMatrix.postScale(scaleFactor, scaleFactor, getWidth() / 2, getHeight() / 2);
        executeTranslate();
    }
	 ...
	 *            旋转角度
	 */
	public void handRotate(float angle) {
		canRotate = true;
		mRotateListener.onRotate(angle, getWidth() / 2, getHeight() / 2);
		executeTranslate();
	}

	public void canRotate(boolean canRotate) {
	...
	public void hasRotateDegrees(float rotateDegrees) {
		mRotateFlag += rotateDegrees;
		if (canRotate) {
			mDegrees += rotateDegrees;
			if (onPhotoViewRotateListener != null) {
				onPhotoViewRotateListener.onRotate(rotateDegrees,
